...
{
    class EditNextCharacterActionIntegTest : public ::testing::Test, public ::feather::test::TestBase
    {
    protected:
    ...
    TEST_F(EditNextCharacterActionIntegTest, editAtNextCharacterWhenNewFile)
    {
        //before
        ...
        std::shared_ptr<feather::utils::storage::AbstractStorage> storage = createEmptyStorage("editAtNextCharacterWhenNewFile");

        //when
        std::unique_ptr<action::EditNextCharacterAction> action = std::make_unique<action::EditNextCharacterAction>(action::EditNextCharacterActionInput(TEST_UUID, pair(0, 0), storage, printingOrchestrator));

        //then
        ASSERT_EQ(action->execute().getResult(), pair(0, 0));

        //when
        action = std::make_unique<action::EditNextCharacterAction>(action::EditNextCharacterActionInput(TEST_UUID, pair(0, 0), storage, printingOrchestrator));
